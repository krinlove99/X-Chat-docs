...
## 适用群体
- X-Chat V2 聊天助手是BASETEC为有二次分发需求的用户/组织提供的X-Chat聊天版本，采用令牌发卡制，同时也对普通用户提供服务。

- X-Chat V2 支持中小型企业、组织及用户对外提供商用服务，通过为用户售卖卡密来使用AI聊天服务，同时企业及组织也可以通过API后台进行审计及管理。

- X-Chat V2 提供弱品牌标识版本，即不含明显的BASETEC品牌标识。同时也提供自定义更换你的品牌标识的服务，详情请咨询 `business@basetec.top` 。

::: warning 注意
 截止于2024年6月20日， `X-Chat V2 聊天助手` 已不再为 **个人用户** 提供更新和维护支持，目前主要服务于使用发卡模式的组织用户。普通用户如需更轻量且强大的聊天客户端，建议前往[X-Chat V3 聊天助手](./X-Chat V3 聊天助手.md)。
...
- 再次点击菜单栏的 `我的令牌` ，如果令牌已生效，点击下拉列表将出现你刚刚创建的令牌。选择一个令牌，会在下方出现该令牌的详细信息。点击 `复制令牌` ，具体配置请参考下方介绍。
